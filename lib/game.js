(function () {
  ...
  Game.prototype.callWave = function (){
    ...
    if(this.waveNum <= 1){
      window.setTimeout( function (){
        for(var j = 0; j < Game.WAVES[this.waveNum]; j++){
          var enemy = new BHGame.Enemy1([Game.WAVE_POSITIONS[this.waveNum][j], 1], this, 100);
    ...
    if(this.waveNum === 1){
      window.setTimeout( function (){
        for(var j = 0; j < Game.WAVES[this.waveNum+1]; j++){
          var enemy = new BHGame.Enemy1([Game.WAVE_POSITIONS[this.waveNum+1][j], 1], this, 0);
          this.enemies.push(enemy);
          this.enemiesRemaining++;
        };
        this.waveNum++;
    ...
    if(this.waveNum === 3){
      window.setTimeout( function (){
        for(var j = 0; j < Game.WAVES[this.waveNum]; j++){
          var enemy = new BHGame.Enemy2([Game.WAVE_POSITIONS[this.waveNum][j], 1], this, 100);
          this.enemies.push(enemy);
          this.enemiesRemaining++;
        };
      }.bind(this), 1000);
      window.setTimeout( function (){
        for(var j = 0; j < Game.WAVES[this.waveNum+1]; j++){
          var enemy = new BHGame.Enemy2([Game.WAVE_POSITIONS[this.waveNum+1][j], 1], this, 0);
          this.enemies.push(enemy);
          this.enemiesRemaining++;
        };
        this.waveNum++;
  ...
  Game.prototype.draw = function (ctxt) {
    ...
    var bgImg = new Image();
    //source : http://eggnored.tumblr.com/stars (not original)
    bgImg.src = "img/bg.png";
    var pattern = ctxt.createPattern(bgImg, 'repeat');

    ctxt.rect(0, 0, Game.DIM_X, Game.DIM_Y);
