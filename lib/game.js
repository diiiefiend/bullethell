(function () {
  ...
  Game.prototype.callWave = function (){
    ...
    if(this.waveNum <= 1){
      var timeoutId = window.setTimeout( function (){
        for(var j = 0; j < Game.WAVES[this.waveNum]; j++){
          var enemy = new BHGame.Enemy1([Game.WAVE_POSITIONS[this.waveNum][j], -window.BHGame.Enemy1.RADIUS], this, 100);
    ...
    if(this.waveNum === 1){
      var timeoutId = window.setTimeout( function (){
        for(var j = 0; j < Game.WAVES[this.waveNum+1]; j++){
          var enemy = new BHGame.Enemy1([Game.WAVE_POSITIONS[this.waveNum+1][j], -window.BHGame.Enemy1.RADIUS], this, 0);
          this.enemies.push(enemy);
          this.enemiesRemaining++;
        };
        this.activeTimeoutIds.pop(timeoutId);
    ...
    if(this.waveNum === 3){
      var timeoutId = window.setTimeout( function (){
        for(var j = 0; j < Game.WAVES[this.waveNum]; j++){
          var enemy = new BHGame.Enemy2([Game.WAVE_POSITIONS[this.waveNum][j], -window.BHGame.Enemy2.RADIUS], this, 100);
          this.enemies.push(enemy);
          this.enemiesRemaining++;
        };
        this.activeTimeoutIds.pop(timeoutId);
